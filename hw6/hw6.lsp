...
; Returns single clause for the constraint:
; "node n gets at most one color from the set {c,c+1,...,k}."
;
(defun at-most-one-color-helper (n c1 c2 k)
  (cond 
    ((= c2 k) NIL)
    (T (cons (list (- (node2var n c1 k)) (- (node2var n (+ c2 1) k))) (at-most-one-color-helper n c1 (+ c2 1) k)))
  )
)
...
(defun at-most-one-color (n c k)
  (cond 
    ((= c k) NIL)
    (T (append (at-most-one-color-helper n c c k) (at-most-one-color n (+ c 1) k)))
  )
)
...
; Returns clause for single edge.
;
(defun generate-edge-clauses-helper (n1 n2 c k)
  (cond 
    ((> c k) NIL)
    (T (cons (list (- (node2var n1 c k)) (- (node2var n2 c k))) (generate-edge-clauses-helper n1 n2 (+ c 1) k)))
  )
)
